...
func TestInit(t *testing.T) {
	...
	msg = <-msgCh
	assert.Equal(t, 1000, len(msg.Data.(*types.Blocks).Items))
...
func initEnv(q queue.Queue) error {
	...
	env1 := protocol.P2PEnv{
		QueueClient: client1,
		Host:        host1,
		Discovery:   dht.InitDhtDiscovery(host1, nil, &types2.P2PSubConfig{Channel: 888}, true),
		DB:          newTestDB(),
	}
	Init(&env1)
	...
	env2 := protocol.P2PEnv{
		QueueClient: client2,
		Host:        host2,
		Discovery: dht.InitDhtDiscovery(host2, nil, &types2.P2PSubConfig{
