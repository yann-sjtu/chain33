...
func (p *Protocol) getChunkRecords(param *types.ReqChunkRecords) *types.ChunkRecords {
	...
	for _, pid := range p.ShardHealthyRoutingTable.ListPeers() {
		records, err := p.getChunkRecordsFromPeer(param, pid)
		if err != nil {
			log.Error("getChunkRecords", "peer", pid, "addrs", p.Host.Peerstore().Addrs(pid), "error", err, "start", param.Start, "end", param.End)
			continue
		}

		log.Info("getChunkRecords", "peer", pid, "addrs", p.Host.Peerstore().Addrs(pid), "start", param.Start, "end", param.End)
		return records
	}
